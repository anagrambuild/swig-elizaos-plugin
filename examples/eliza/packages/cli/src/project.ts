...
import { character as swigAgentCharacter } from '@/src/characters/swig-agent';
  ...
  try {
    ...
    if (moduleIsPlugin) {
      ...
      try {
        ...
        // Choose the appropriate character based on the plugin
        const baseCharacter = completePlugin.name.toLowerCase().includes('swig')
          ? swigAgentCharacter
          : elizaCharacter;

        const characterName = completePlugin.name.toLowerCase().includes('swig')
          ? 'SwigAgent (Test Mode)'
          : 'Eliza (Test Mode)';
        ...
        const testCharacter: Character = {
          ...
          id: uuidv4() as UUID,
          name: characterName,
          system: `${baseCharacter.system} Testing the plugin: ${completePlugin.name}.`,
        };

        logger.info(
          `Using ${baseCharacter.name} character as test agent for plugin: ${completePlugin.name}`
        );
