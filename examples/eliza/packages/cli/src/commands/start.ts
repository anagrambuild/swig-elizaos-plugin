...
import { getSwigAgentCharacter } from '@/src/characters/swig-agent';
    ...
    if (isProject && projectModule?.default) {
      ...
      if (agents.length > 0) {
        ...
        if (startedAgents.length === 0) {
          logger.info('No project agents started - starting default characters');
          const elizaCharacter = getElizaCharacter();
          const swigCharacter = getSwigAgentCharacter();
          await startAgent(elizaCharacter, server);
          await startAgent(swigCharacter, server);
        } else {
          logger.info(`Successfully started ${startedAgents.length} agents from project`);
        }
      } else {
        logger.info('Project found but no agents defined, starting default characters');
        const elizaCharacter = getElizaCharacter();
        const swigCharacter = getSwigAgentCharacter();
        await startAgent(elizaCharacter, server);
        await startAgent(swigCharacter, server);
      ...
      // When not in a project or plugin, start both default characters
      const elizaCharacter = getElizaCharacter();
      const swigCharacter = getSwigAgentCharacter();
      logger.debug(`Starting default characters: ${elizaCharacter.name} and ${swigCharacter.name}`);
      await startAgent(elizaCharacter, server);
      await startAgent(swigCharacter, server);
    }
